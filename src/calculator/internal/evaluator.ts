...
export default function evaluate(tokens: Token[], ans: Decimal, ind: Decimal, angleUnit: AngleUnit): EvalResult {
	 ...
	 */
	function expect(pattern: Pattern.Pattern<Token>, consumeNext = true): Result<Token, EvalErrorId> {
		const token = peek();
				...
				evalArgs().andThen(args =>
					match(token.name)
						.with("root", () => {
							...
							let radicand = args[0]!;
							let degree = args[1] ?? TWO;

							if (degree.lessThan(1) || !degree.isInteger()) {
								return err("NOT_A_NUMBER" as const);
							}

							return ok(radicand.isZero() ? radicand : radicand.pow(ONE.div(degree)));
						})
						.otherwise(funcName => {
							if (args.length < 1) return err("NOT_ENOUGH_ARGS" as const);
							if (args.length > 1) return err("TOO_MANY_ARGS" as const);

							let func = Decimal[funcName].bind(Decimal);
							let arg = args[0]!;
